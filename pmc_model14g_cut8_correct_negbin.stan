// Both trial arms - fit.
// changing prob infection with EIR
// fit to daily data
// let prob sm be different in PMC and placebo
// correct rates all = daily transition probability.
...
functions {  // runs model for one site.
  ...
  /// INITIAL CONDITIONS
    ...
    if(i<pmc_times[1]) {
      ...
      left_um_pmc[i] = prop_fup_pmc[i-1]*(prob_symp_d[i-1]*al_eff*S_pmc[i-1]*prob_um_plac);
      left_sev_pmc[i] = prop_fup_pmc[i-1]*(prob_symp_d[i-1]*al_eff*S_pmc[i-1]*(1-prob_um_plac));
      ...
      res[i-1,4] = prob_symp_d[i-1]*S_pmc[i-1]*prob_um_plac;
          ...
          left_um_pmc[i] = prop_fup_pmc[i-1]*(prob_symp_d[i-1]*al_eff*S_pmc[i-1]*(1-current_pp_dp)*current_prob_um);
          left_sev_pmc[i] = prop_fup_pmc[i-1]*(prob_symp_d[i-1]*al_eff*S_pmc[i-1]*(1-current_pp_dp)*(1-current_prob_um));
...
model{
  ...
  //target += gamma_lpdf(EIR_y | shape_EIR_y, rate_EIR_y);
  EIR_y ~ gamma(shape_EIR_y, rate_EIR_y);
  r ~ normal(0, 1);
  ...
  for(s in 1:nSites) {
    for(j in 1:4) {
      //target += poisson_lpmf(daily_events[,j,s] | res[15:runTime,j,s]);
      daily_events[,j,s] ~ neg_binomial_2(res[15:runTime,j,s], k);
      //daily_events[,j,s] ~ neg_binomial_2_log(res[15:runTime,j,s], k);
    }
  }
}
...
//       max_beta_phi, r, runTime, 50,
//       50, prob_rp, pmc_times,
//       p_protect_dp);
//   }  
// 
//   // no need to compute within stan.
//   // for(i in 1:nPred) {
//   //   for(j in 1:8) {
//   //     inc[i,j]=res2[i,j]/py[j];
//   //   }
//   // } 
//   // 
//   // for(i in 1:nPred) {
//   //   inc_tot[i,1]=(res2[i,1]+res2[i,2])/(py[1]+py[2]); // UM months 1-6 PMC
//   //   inc_tot[i,2]=(res2[i,3]+res2[i,4])/(py[3]+py[4]); // SM months 1-6 PMC
//   //   inc_tot[i,3]=(res2[i,5]+res2[i,6])/(py[5]+py[6]); // UM months 1-6 plac
//   //   inc_tot[i,4]=(res2[i,7]+res2[i,8])/(py[7]+py[8]); // SM months 1-6 plac
//   // } 
// }
