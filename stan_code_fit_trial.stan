// STAN FILE - model to fit post discharge cohort model to trial data.
// rename R1 to Um?
// check if incoming not used anymore.

functions {  
  // runs model for one site.
  real[,] run_model(real EIR_y, real prob_um_plac, real prob_um_pmc, real w_scale_risk, 
  real w_shape_risk, real max_beta_phi, real r, int runTime, int N_plac_init,
  ...
  dur_sev=dur_p+3;  // dur_p + 3 days median hospital stay
  /// INITIAL CONDITIONS
  //changing incidence over time since discharge.
    for(i in 1:runTime) {
    ...
    /// Run placebo arm
      ...
      // store new episodes per day in res output
      // hospitalised episodes
    ...
    if(i<pmc_times[1]) {
      ...
      res[i-1,4] = prob_symp_d[i-1]*S_pmc[i-1]*prob_um_plac;
          ...
          // allow for adherence to each dose in Kwambai et al.2020
          ...
          // allow probability of uncomplicated vs hospitalised episode could be different when drug-protected
...
data{
  ...
  // dimensions of daily events: time, 4 outcome groups (2 trt groups x 2 outcomes (UM,SM)) x nSites
